...
public class KandleStructure<E extends Comparable<E>>{
    ...
    //Number of Packages that would be hold on a container (third level+)
    private int PACK_N_PACKAGES = 4;
    ...
    public void packing(){
        ...
        }else if(countPackages == PACK_N_PACKAGES){
            Kandle k = new KandlePackage(ipp, fpp);

            packingReference(k, ipp, fpp);

            countPackages -= 3;
        }
    }

    private static void packingReference(Kandle k, Kandle ip, Kandle fp){
        k.prev = ip.prev;
        ip.prev = k;
        k.next = fp.next;
        fp.next = k;
